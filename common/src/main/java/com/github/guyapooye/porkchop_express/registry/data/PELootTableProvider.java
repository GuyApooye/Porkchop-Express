...
public class PELootTableProvider implements LootTableSubProvider {
    ...
    public PELootTableProvider(HolderLookup.Provider registryLookup) {
    ...
    private <T extends Block> void dropSelf(RegistryObject<T> block) {
        ...
        this.generateBlock(
                block.getId(),
                () -> PELootTableProvider.dropSelf((Supplier<T>) block)
        );
    }
    ...
    private static ResourceKey<LootTable> resourceKey(String path) {
        return resourceKey(PorkchopExpress.id(path));
    }
